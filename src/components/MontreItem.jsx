...
function MontreItem({ montre }) {
    return (
        <tr className="h-[75px] border-b border-t overflow-hidden">
            ...
            <td>
                <div className="flex items-center">
                    ...
                    <div className="w-auto font-bold">
                        <Link to={`/montre/${montre.id}`}>{montre.name}</Link>
                    </div>
                    <div className="text-xs ml-2 text-hsla-gray dark:text-0.6">
                        <Link to={`/montre/${montre.id}`}>
                            {montre.symbol.toUpperCase()}
                        </Link>
                    </div>
                </div>
            </td>
            ...
            <td className="w-40 text-sm py-1 px-2">
                ${montre.current_price.toLocaleString()}
            </td>
            <td
                className={
                    montre.price_change_percentage_24h > 0
                        ? "text-green-500 w-40 text-sm"
                        : "text-red-600 w-40 text-sm"
                }
            >
                {montre.price_change_percentage_24h.toFixed(2)}%
            </td>
            <td>
